...
/**
 * {@link CopyOnWriteArrayBag}'s unit tests.
 *
 * @author Osman KOCAK
 */
public final class CopyOnWriteArrayBagTest
{
	@Test
	public void testIterableContructor()
	{
		Iterable<String> iterable = Arrays.asList("Hello", "World");
		Bag<String> bag = new CopyOnWriteArrayBag<String>(iterable);
	...
	public void testIteratorContructor()
	{
		...
		Bag<String> bag = new CopyOnWriteArrayBag<String>(collection.iterator());
		assertEquals(2, bag.size());
		assertTrue(bag.contains("Hello"));
		assertTrue(bag.contains("World"));
	}
	...
	{
		Bag<String> bag = new CopyOnWriteArrayBag<String>("Hello", "World");
		assertEquals(2, bag.size());
		assertTrue(bag.contains("Hello"));
		assertTrue(bag.contains("World"));
	}
	...
	public void testAdd()
	{
		Bag<String> bag = new CopyOnWriteArrayBag<String>();
		bag.add("Hello");
		assertTrue(bag.contains("Hello"));
	}
	...
	{
		ConcurrentBag<Long> bag = new CopyOnWriteArrayBag<Long>(1L, 2L);
	...
	{
		assertTrue(new CopyOnWriteArrayBag<Integer>().isEmpty());
	...
	{
		assertFalse(new CopyOnWriteArrayBag<Integer>().iterator().hasNext());
		List<Long> in = Arrays.asList(1L, 2L, 1L, 2L, 3L);
		Bag<Long> bag = new CopyOnWriteArrayBag<Long>(in);
	...
	{
		Bag<Long> bag = new CopyOnWriteArrayBag<Long>(1L, 2L, 1L);
	...
	{
		assertFalse(new CopyOnWriteArrayBag<Long>(1L, 5L, 1L).removeAll(Arrays.asList(2L)));
		Bag<Long> bag = new CopyOnWriteArrayBag<Long>(1L, 2L, 1L, 3L);
	...
	{
		assertFalse(new CopyOnWriteArrayBag<Long>(2L, 2L).retainAll(Arrays.asList(2L)));
		Bag<Long> bag = new CopyOnWriteArrayBag<Long>(1L, 2L, 1L, 3L);
	...
	{
		assertEquals(0, new CopyOnWriteArrayBag<String>().toArray().length);
		String[] in = new String[]{"Hello", "World"};
		String[] out = new CopyOnWriteArrayBag<String>(in).toArray(new String[0]);
	...
	{
		Bag<Integer> bag1 = new CopyOnWriteArrayBag<Integer>(1, 2, 3, 1, 2, 3);
		Bag<Integer> bag2 = new CopyOnWriteArrayBag<Integer>(3, 1, 2, 3, 1, 2);
		Bag<Integer> bag3 = new CopyOnWriteArrayBag<Integer>(2, 3, 1, 2, 3, 1);
